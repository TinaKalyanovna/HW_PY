...
def change_records(file_name: str):
    ...
    if record_id != 'q':
        replaced_line = f'{int(record_id)};' + ';'.join(
            input('Введите изменения: фамилию, имя, отчество, номер телефона через пробел\n').split()[:4]) + ';\n'
...
try:                        # исключения try/except/finally # открыть файл # если нет файла он создается
    file = open(path, 'r')  
except IOError:             
    print('Создан телефонный справочник -> phone_book.txt ')
    file = open(path, 'w')
finally:                    
    file.close()
